...
void GuiWindow::RemoveAll() { _elements.clear(); }
...
u32 GuiWindow::GetSize() { return _elements.size(); }
...
void GuiWindow::ToggleFocus(GuiTrigger *t) {
    ...
    if (found == -1) {
        for (i = 0; i < elemSize; ++i) {
            try {
                if (_elements.at(i)->IsFocused() == 0 &&
                    _elements.at(i)->GetState() !=
                        STATE_DISABLED) // focus is possible (but not set)
                {
    ...
    // change focus
    else if (t->wpad->btns_d &
                 (WPAD_BUTTON_1 | WPAD_BUTTON_B | WPAD_CLASSIC_BUTTON_B) ||
        ...
        for (i = found; i < elemSize; ++i) {
            try {
                        ...
                        STATE_DISABLED) // focus is possible (but not set)
                {
                    newfocus = i;
                    _elements.at(i)->SetFocus(1); // give this element focus
                    _elements.at(found)->SetFocus(
                        0); // disable focus on other element
                    break;
                }
            } catch (const std::exception &e) {
            }
    ...
    for (i = 0; i < elemSize; ++i) {
        try {
            if (_elements.at(i)->IsSelectable()) {
                if (_elements.at(i)->GetLeft() * dir > left * dir &&
                    _elements.at(i)->GetTop() == top) {
                    if (found == -1)
                        found = int(i);
                    else if (_elements.at(i)->GetLeft() * dir <
                             _elements.at(found)->GetLeft() * dir)
        ...
        try {
            if (_elements.at(i)->IsSelectable()) {
                if (_elements.at(i)->GetTop() * dir > top * dir) {
                    if (found == -1)
                        found = i;
                    else if (_elements.at(i)->GetTop() * dir <
                             _elements.at(found)->GetTop() * dir)
                        found = i; // this is a better match
                    else if (_elements.at(i)->GetTop() * dir ==
                                 _elements.at(found)->GetTop() * dir &&
                             _elements.at(i)->GetLeft() * dir <
                                 _elements.at(found)->GetLeft() * dir)
                        found = i; // this is a better match
                }
            }
