...
static void pngu_free_info(IMGCTX ctx) {
    if (ctx->infoRead) {
        ...
        png_destroy_read_struct(&(ctx->png_ptr), &(ctx->info_ptr),
                                (png_infopp)NULL);
...
static void pngu_read_data_from_buffer(png_structp png_ptr, png_bytep data,
                                       png_size_t length) {
    IMGCTX ctx = (IMGCTX)png_get_io_ptr(png_ptr);
    memcpy(data, reinterpret_cast<u8*>(ctx->buffer) + ctx->cursor, length);
    ctx->cursor += length;
}

// Custom data writer function used for writing to memory buffers.
static void pngu_write_data_to_buffer(png_structp png_ptr, png_bytep data,
                                      png_size_t length) {
    IMGCTX ctx = (IMGCTX)png_get_io_ptr(png_ptr);
    memcpy(reinterpret_cast<u8*>(ctx->buffer) + ctx->cursor, data, length);
    ctx->cursor += length;
}
...
static int pngu_info(IMGCTX ctx) {
    ...
    if (!(ctx->info_ptr)) {
        if (ctx->source == PNGU_SOURCE_DEVICE)
            fclose(ctx->fd);
        png_destroy_read_struct(&(ctx->png_ptr), (png_infopp)NULL,
                                (png_infopp)NULL);
        return PNGU_LIB_ERROR;
    }
    ...
    if (ctx->source == PNGU_SOURCE_BUFFER) {
        ...
        // Default data provider uses function fread, so it needs to use our
        // FILE*
        png_init_io(ctx->png_ptr, ctx->fd);
        png_set_sig_bytes(
            ctx->png_ptr,
            8); // We have read 8 bytes already to check PNG authenticity
    }
    ...
    if (!ctx->propRead) {
        ...
        png_get_IHDR(ctx->png_ptr, ctx->info_ptr, &width, &height,
                     ...
                     (int *)&(ctx->prop.imgColorType), NULL, NULL, NULL);
        ...
        case PNGU_COLOR_TYPE_RGB_ALPHA: {
            ...
            if (png_get_tRNS(ctx->png_ptr, ctx->info_ptr, &trans,
                             (int *)&(ctx->prop.numTrans), &trans_values)) {
                ctxNumTrans = ctx->prop.numTrans;
                if (ctxNumTrans) {
                    ctx->prop.trans = reinterpret_cast<PNGUCOLOR*>(malloc(sizeof(PNGUCOLOR) * ctxNumTrans));
                    if (ctx->prop.trans)
                        for (i = 0; i < ctxNumTrans; i++) {
                            ctx->prop.trans[i].r = trans_values[i].red >> scale;
                            ctx->prop.trans[i].g =
                                trans_values[i].green >> scale;
                            ctx->prop.trans[i].b =
                                trans_values[i].blue >> scale;
                        }
                    else
                        ctx->prop.numTrans = 0;
                }
        ...
        case PNGU_COLOR_TYPE_GRAY_ALPHA: {
            if (png_get_bKGD(ctx->png_ptr, ctx->info_ptr, &background)) {
                ctx->prop.validBckgrnd = 1;
                ctx->prop.bckgrnd.r = ctx->prop.bckgrnd.g =
            ...
            if (png_get_tRNS(ctx->png_ptr, ctx->info_ptr, &trans,
                ...
                if (ctxNumTrans) {
                    ...
                    if (ctx->prop.trans)
                        for (i = 0; i < ctxNumTrans; i++)
                            ctx->prop.trans[i].r = ctx->prop.trans[i].g =
                                ctx->prop.trans[i].b =
...
static int pngu_decode(IMGCTX ctx, PNGU_u32 width, PNGU_u32 height,
                       PNGU_u32 stripAlpha) {
    ...
    if (stripAlpha && ((ctx->prop.imgColorType == PNGU_COLOR_TYPE_RGB_ALPHA) ||
                       (ctx->prop.imgColorType == PNGU_COLOR_TYPE_GRAY_ALPHA)))
    ...
    if ((ctx->prop.imgColorType == PNGU_COLOR_TYPE_GRAY) ||
        (ctx->prop.imgColorType == PNGU_COLOR_TYPE_GRAY_ALPHA))
    ...
    if (rowbytes & 3)
        rowbytes =
            ((rowbytes >> 2) + 1)
            << 2; // Add extra padding so each row starts in a 4 byte boundary

    ctx->img_data = reinterpret_cast<png_bytep>(malloc(rowbytes * ctx->prop.imgHeight));
    if (!ctx->img_data) {
        pngu_free_info(ctx);
        return PNGU_LIB_ERROR;
    }

    ctx->row_pointers = reinterpret_cast<png_bytep*>(malloc(sizeof(png_bytep) * ctx->prop.imgHeight));
    if (!ctx->row_pointers) {
        free(ctx->img_data);
        pngu_free_info(ctx);
        return PNGU_LIB_ERROR;
...
static inline PNGU_u32 coordsRGBA8(PNGU_u32 x, PNGU_u32 y, PNGU_u32 w) {
    return ((((y >> 2) * (w >> 2) + (x >> 2)) << 5) + ((y & 3) << 2) + (x & 3))
           << 1;
}

static u8 *PNGU_DecodeTo4x4RGBA8(IMGCTX ctx, PNGU_u32 width, PNGU_u32 height,
                                 int *dstWidth, int *dstHeight, int maxWidth,
                                 int maxHeight) {
    ...
    dst = reinterpret_cast<u8*>(memalign(32, len));
...
IMGCTX PNGU_SelectImageFromBuffer(const void *buffer) {
    ...
    ctx = reinterpret_cast<_IMGCTX*>(malloc(sizeof(struct _IMGCTX)));
    if (!ctx)
        return NULL;
...
IMGCTX PNGU_SelectImageFromDevice(const char *filename) {
    ...
    ctx->filename = reinterpret_cast<char*>(malloc(strlen(filename) + 1));
    if (!ctx->filename) {
        free(ctx);
        return NULL;
    }
...
PNGU_u8 *DecodePNG(const PNGU_u8 *src, int *width, int *height, int maxwidth,
                   int maxheight) {
    PNGUPROP imgProp;
...
int PNGU_EncodeFromRGB(IMGCTX ctx, PNGU_u32 width, PNGU_u32 height,
                       void *buffer, PNGU_u32 stride) {
    png_uint_32 rowbytes;
    PNGU_u32 y;
    ...
    if (ctx->source == PNGU_SOURCE_BUFFER) {
        ...
        ctx->cursor = 0;
        png_set_write_fn(ctx->png_ptr, ctx, pngu_write_data_to_buffer,
                         pngu_flush_data_to_buffer);
    } else if (ctx->source == PNGU_SOURCE_DEVICE) {
        // Default data writer uses function fwrite, so it needs to use our
        // FILE*
        png_init_io(ctx->png_ptr, ctx->fd);
    }

    // Setup output file properties
    png_set_IHDR(ctx->png_ptr, ctx->info_ptr, width, height, 8,
    ...
    ctx->img_data = reinterpret_cast<png_bytep>(malloc(rowbytes * height));
    ...
    ctx->row_pointers = reinterpret_cast<png_bytep*>(malloc(sizeof(png_bytep) * height));
    ...
    for (y = 0; y < height; ++y) {
        ctx->row_pointers[y] = reinterpret_cast<u8*>(buffer) + (y * rowbytes);
    }
...
int PNGU_EncodeFromGXTexture(IMGCTX ctx, PNGU_u32 width, PNGU_u32 height,
                             void *buffer, PNGU_u32 stride) {
    int res;
